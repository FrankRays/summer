...
<body>
    ...
    <form action="<?php echo site_url('admin/friendlink/doedit') ?>" method="POST" onsubmit="return chkSub();">
    <ul class="forminfo">
    <li><label>链接名称</label><input id="name" value="<?php echo $data['name'] ?>" name="name" type="text" class="dfinput" /><i>标题不能超过30个字符</i></li>
    <li><label>链接地址</label><input id="url" name="url" value="<?php echo $data['link_url'] ?>" type="text" class="dfinput" /></li>
    <li><label>&nbsp;</label><input id="btn" name="" type="submit" class="btn" value="确认保存"/></li>
    <input type="hidden" id="student-id" name="id" value="<?php echo $data['id'] ?>"/>
    </ul>
    </form>
    </div>
