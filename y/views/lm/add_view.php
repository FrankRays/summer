...
<body>
    ...
    <ul class="placeul">
    <li><a href="#">首页</a></li>
    <li><a href="<?php echo site_url('admin/lm/li')?>">栏目管理</a></li>
    <li><a href="#">栏目添加</a></li>
    </ul>
    </div>
    ...
    <div class="formtitle"><span>栏目添加</span></div>
    <form action="<?php echo site_url('admin/lm/doAdd') ?>" method="POST" />
	    <ul class="forminfo">
	    <li><label>选择本站分类</label><select name="cid" class="df-select">
	    <option value="0">链接外站</option>
	    <?php foreach($categoryList as $v){ ?>
	    	<option value="<?php echo $v['id'] ?>"><?php echo str_repeat('&nbsp;&nbsp;&nbsp;&nbsp;', ($v['lev']-1)).$v['name'] ?></option>
	    <?php } ?>
	    </select></li>
	    <li><label>栏目名称</label><input id="name" name="name" type="text" class="dfinput" /><i>非外站链接勿用</i></li>
	     <li><label>栏目链接</label><input id="link_src" name="link_src" type="text" class="dfinput" /><i>非外站链接勿用</i></li>
