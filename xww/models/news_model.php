...
class news_model extends CI_Model{
	...
	public function getByPageByAlias($offset, $limit, $alias){
		$this -> db -> where(array(
			'news_category.alias' => $alias
			));
		$this -> db -> select("news.id as newsID, title, news.add_time as addTime");

		$this -> db -> join('news_category', 'news_category.id = news.category_id', 'cross');
		$this -> db -> order_by('news.add_time', 'DESC');
	...
	//set the topnews
	public function topNews($offset, $limit, $alias, $isTop){
		$where = array(
			'news_category.alias' => $alias,
			'news.is_top' => $isTop,
			'news.is_delete' => 0,
		...
		$this -> db -> select("news.id as newsID, title,  news.add_time as addTime, news.pic_src");
		...
		foreach ($result as $key => $value) {
			...
			if(mb_strlen($value['title']) > 14){
				$result[$key]['title'] = mb_substr($value['title'], 0, 14) . ".." ;
			}
		}
		return $result;
	...
	**/
	public function getById($id){
		...
		$select = 'news.id as news_id, zan,';
		$select .= 'news.hits, news_category.id as category_id, news.status,';
		$select .= 'news_category.name as category_name, video_src, is_video, ';
		$select .= 'news_category.describle, ';
...
/*
	...
	**/
	...
	public function getPhotoNewsByCond($limit, $offset, $cond = array()){			
		$cond['is_delete'] = 0;
		$cond['status'] = 1;
	...
	public function getPhotoById($id){
		...
		$result['photoes'] = json_decode($result['content']);
		return $result;
	}
	...
	public function add_zan($id){
		...
		$news = $this->db->select("id, zan")
				->where('id', $id)
				->get($this->tableName, 1)
				->result_array();
		...
		if(empty($news[0]['zan']))	$news[0]['zan'] = 0;
		$news[0]['zan'] = intval($news[0]['zan']) + 1;

		$this->db->where('id', $id)->update($this->tableName, array('zan' =>$news[0]['zan']));
		return true;
	} 
