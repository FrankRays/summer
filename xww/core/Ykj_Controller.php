...
class Ykj_Controller extends CI_Controller{
	...
	public function judgeLogin($login = FALSE){
		$session_Clogin = $this -> session -> userdata('Clogin');
		$cookie_Clogin = $this -> input ->	cookie('Clogin', TRUE);

		if(empty($session_Clogin) || $session_Clogin['life'] < time()){
			if(empty($session_Clogin)){
				if($login == TRUE){
					return TRUE;
				}
				$msg = '你还没有登陆';
				...
				$this -> session -> set_userdata(array('Clogin' => array(
						'email' 	=>		$cookie_Clogin,
						'life'		=>		time() + 60 * 60 * 12,
					)));
