	...
	<div id="recent-news-con">
		<div id="recent-news-left">
			<div id="location-bar">
				<p><i  class="fi-marker location-icon"></i>您的位置：<a id="pos1" href="<?php echo site_url() ?>" >交院新闻网</a> > <?php echo $categoryList['name'] ?></p>
			</div>
			<?php foreach($newsList as $v){ ?>
				<div style="height:25px;" class="recent-news-wrap">
					<p class="tit"><a href="<?php echo site_url('news/archive/'.$v['news_id']) ?>"><?php echo $v['title'] ?></a> <span style="float:right;padding-top:2px;"><?php echo $v['add_time'] ?></span></p>
				</div>
			<?php } ?>
