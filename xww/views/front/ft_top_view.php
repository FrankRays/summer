...
<?php
    if(isset($head)){
        if(isset($head['css'])){
            foreach ($head['css'] as $key => $value) {
                echo '<link rel="stylesheet" href="'.base_url($head['basepath'].$value).'" />'; 
            }
        }

        if(isset($head['js'])){
            foreach ($head['js'] as $key => $value) {
                echo '<script src="'.base_url($head['basepath'].$value).'"></script>';
            }
        }

        if(isset($head['new_js'])){
            foreach ($head['new_js'] as $key => $value) {
                echo '<script src="'.base_url($head['basepath'].$value).'"></script>';
            }
        }

        if(isset($heade['new_css'])){
            foreach ($head['new_css'] as $key => $value) {
                echo '<link rel="stylesheet" href="'.base_url($head['basepath'].$value).'" />';
            }
        }
    }
